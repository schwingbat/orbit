...
const Wheel = new Component({
  ...
  postrender() {
    ...
    function handleMouseMove(e) {
      if (state.changingHue) {
        ...
        const x =
          (e.touches ? e.touches[0].clientX : e.clientX) - wheelCenter.x;
        const y =
          (e.touches ? e.touches[0].clientY : e.clientY) - wheelCenter.y;
      ...
      if (state.changingSat) {
        ...
        const sAmount = Math.max(
          0,
          Math.min(
            1,
            ((e.touches ? e.touches[0].clientX : e.clientX) - sRect.left) /
              sRect.width
          )
        );

        self.nodes.saturationKnob.style.background = `hsl(${state.hue * 360}, ${
          sAmount * 100
        }%, 50%)`;
      ...
      if (state.changingLight) {
        ...
        const lAmount = Math.max(
          0,
          Math.min(
            1,
            ((e.touches ? e.touches[0].clientX : e.clientX) - lRect.left) /
              lRect.width
          )
        );

        self.nodes.lightnessKnob.style.background = `hsl(${
          state.hue * 360
        }, 0%, ${lAmount * 100}%)`;
  ...
  updaters: {
    ...
    changingSat: function (val) {
      ...
      if (val) {
        knob.classList.add("active");
        knob.style.background = `hsl(${(hue * 360).toFixed(1)}, ${(
          saturation * 100
        ).toFixed(1)}%, 50%)`;
      } else {
        knob.classList.remove("active");
    ...
    changingLight: function (val) {
      ...
      if (val) {
        knob.classList.add("active");
        knob.style.background = `hsl(${(hue * 360).toFixed(1)}, 0%, ${(
          lightness * 100
        ).toFixed(1)}%)`;
      } else {
        knob.classList.remove("active");
...
const Formats = new Component({
      ...
      if (e.keyCode === 13) {
        ...
        switch (el.id) {
          ...
          case "hsl-value":
            ...
            break;
          case "rgb-value":
            rgb = el.value.split(",").map((c) => parseInt(c));
  ...
  updaters: {
    ...
    validHex: function (val) {
      this.nodes.hexValue.parentNode.classList[val ? "remove" : "add"](
        "invalid"
      );
    },
    validRGB: function (val) {
      this.nodes.rgbValue.parentNode.classList[val ? "remove" : "add"](
        "invalid"
      );
    },
    validHSL: function (val) {
      this.nodes.hslValue.parentNode.classList[val ? "remove" : "add"](
        "invalid"
      );
    },
  },
...
const Orbit = new Component({
  ...
  render(el) {
    return el("div", { class: "orbit-container" }, [
      ...
      el("div", { class: "controls-container" }, [Wheel.el, Formats.el]),
    ]);
  },
  postrender() {
    ...
    if (window.location.hash.indexOf("#") !== -1) {
      this.update({
        color: color.rgbToHSL(
          color.hexToRGB("#" + window.location.hash.split("#").pop())
        ),
      });
    } else {
      // Load saved color.
    ...
    console.log(
      `Orbit initialized and rendered in ${Date.now() - loadedTime}ms.`
    );
  },
  events: {
    "click .download-swatch": function (e) {
      ...
      a.setAttribute("download", "swatch-" + c.hex.slice(1) + ".png");
